...
\newtext{
The protocol starts with approaching vehicles receiving messages from existing pending vehicles. An approaching may not start broadcasting it's own messages until it is within a 'lurk distance' of the intersection. 
}
...
\caption{\newtext{The flowchart for lane changing decisions from \citep{Gipps1986} }}
\end{figure}
...
\newtext{
Atagoziyev's model is designed around autonomous vehicles and as such it takes advantage of the benefits that provides. The model manages a set of vehicles over two lanes, organising their movements into their preferred lanes before they reach a critical position.
} 
...
\newtext{In the paper SV refers to the subject vehicle, the one that wants to change lanes. CL is the vehicle in front of the SV. TL is the vehicle the SV wants to be behind in its target lane. LV is the vehicle that will be behind SV once it moves to its target  lane}

\newtext{Below are the summaries for the seven equations provided in Atagaziyev's paper.
\begin{enumerate}
...
\newtext{
These equations are the building blocks that lead to a lane change, this flowchart shows how they work together to create a single lane change.
}

\begin{figure}[htb]
\includegraphics[width=\textwidth]{AtagoziyevFlowchart.JPG}
\caption{\newtext{The flowchart for Atagoziyev's Lane Changing model equations}}
\end{figure}

\newtext{
Using this algorithm for multiple changes should automatically put all of the vehicles in the correct lanes, as LV always provides space to allow each vehicle that wishes to change to move into the correct lane. All of the vehicles are working with each other such that they can all reach their goal. Comparing this to Gipps' 1986 model, where drivers are acting solely in their own interest, we can see that by having an external agent managing lane changes the autonomous vehicles can achieve results that might not be possible in situations where drivers are acting selfishly. For example, in a grid locked situation, drivers following Gipps' model might never be able to move into their desired lane, however vehicles following Atagoziyev's model would open up spaces allowing vehicles to move through the traffic.
}
...
\newtext{
Again, Gipps' 1986 model was built with human drivers in mind, and as such it fails to take advantage of the benefits of autonomous vehicles. Work by Kesting et al. in 2007 \citep{Kesting2007} describes a decentralised model of lane changing that lets vehicles change lanes to increase velocity whilst still ensuring that the overall traffic flow is not disrupted. This helps to avoid traffic shocks and maintains smooth traffic flow.
}

\newtext{
Kesting introduces the MOBIL or 'Minimising Overall Braking Induced by Lane Changes' model. It is based off a number of criterion. The first being a safety criterion:
}
...
\newtext{
This criterion ensures the deceleration of a successor vehicle $\tilde{a}_n$ in the target lane doesn't exceed a safety limit $b_{safe}$. Essentially this puts a limit on the level of braking a vehicle changing lanes can cause another vehicle to undergo if it pulls out in front of it.
}
...
\newtext{
 It varies based on whether the driver is driving under symmetric ('US') lane-changing rules or asymmetric ('European') passing rules. Under 'US' rules the incentive criterion is:
}
...
\newtext{
For European countries which have rules on which side cars can overtake on we introduce two new terms
}

\begin{equation}
a^{eur}_c = {
	\begin{array}{ll}
		min(a_c, \tilde{a}_c) \text{ if} v_c > \tilde{v}_{lead} > v_{crit}\\
		a_c \text{ otherwise}
	\end{array}
}
\end{equation}

\todo{Finish section on European roads and implementation into IDM. Discuss results of changing $P$ values Compare to Gipps 1986. Model appreciates other drivers, could get accurate braking measurements etc. from them.}